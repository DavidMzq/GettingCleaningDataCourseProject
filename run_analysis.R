# This R code is for generaing a required tidy data set for week3's Course Project for Data Scientist series Getting and Cleaning Data Couse provided by Coursera.
...
# Note:
# 1.There's decriptive text file named README.md and cookbook file along with this sourcecode file which can be found in same github folder.
# 2.Reading the README.md file carefully and know the process as a whole, particularly Unzip downloaded data file into your R work dirctory's subfolder named data or any other subfolder name before running this R file.
# 3.Within this R file, there're lines to install needed packages, but if you want to install those packages in advance, that doesn't matter. 
#   since you may encounter with network problems to download packages during running this file, so install packages below before is encouraged.
#   Applied Package: "data.table", "LaF", "plyr"
# 4. There is a little different between the original required/suggested steps' order and the real running order for easier handle of data.
...
##### Get Train data
datX <- laf_open_fwf(filename="./data/UCI HAR Dataset/train/X_train.txt", column_types=Vec_Column,column_widths=Vec_Width,trim=TRUE)
dfXtrain <- datX[ , ]
...
##### Get Test data
datX <- laf_open_fwf(filename="./data/UCI HAR Dataset/test/X_test.txt", column_types=Vec_Column,column_widths=Vec_Width,trim=TRUE)
dfXtest <- datX[ , ]
...
# As the course project profile suggested, we should do step of "Extracts only the measurements on the mean and standard deviation for each measurement." now, but based on 2 reasons, we could do this later:
#  1) For measurements on the mean and standard deviation, they cuold be found every other 40 of all features, but after position 207 they change a lot, we have to location their position use vector like c(1:7,42:47,82:87,122:127,162:167,202:207,215,216...425:430,503,504,...) 
#     (Notice: the first column now is the Training Label not the first feature after we merge them together).  
#     How could we handle this kind of situation, especially when there are more features, identify their position one by one manually or introduce a 3rd party program? 
#  2) Even we extract all the needed features, we should add each matched feature names for all of them, and we have to use the same position vector again to get those feature names from the features.txt. 
...
# Since we have a combined data frame AllData with the subject and activity as its first 2 columns, so add "Subject" and "Activity" as the first 2 columns' name first.
cnames <- character()
...
colnames(AllData) <- cnames # Add column names with the data set, you can type "names(AllData)" after R prompt to see the whole names

print("Step 3 Ended")
################ Step 4 - Extracts only the measurements on the mean and standard deviation for each measurement. The output data named AllDataExtracted ################
...
# you can type names(AllDataExtracted) to see what we get 68 variables names for all variables contian "mean()" and "std()" as part of their name, and the first 2 are "Subject" and "Activity".

# Notice: There are column names like "tBodyAcc-mean()-X" from original features.txt and following dfFeature, it doesn't matter for the data frame itself, 
# but will affect later calculation, so they need to be modified with normal name.
# Change Rule:
#   1) - to _
#   2) remove ()
...
print("Step 5 Ended")
################ Step 6 - From the data set AllDataExtracted in step5, creates a second, independent tidy data set named DataRequired with the average of each variable for each activity and each subject. ################
...
write.table(DataRequired, file="DataRequired.txt", row.names=FALSE, col.names=FALSE, sep=",", quote=FALSE) # output the required final Data set into a file named DataRequired.txt in the work directoty.
